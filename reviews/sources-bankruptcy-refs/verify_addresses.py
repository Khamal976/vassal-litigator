...
# Дефис — ЧАСТЬ номера статьи (225.10-1), а не разделитель. Без `(?:-\d+)*`
# движок откатывается и берёт «225», из-за чего пять разных статей главы 28.1
# сливались в одну запись «ст. 225 АПК» с чужими пунктами. Тот же дефект был
# в fetch_articles.py и там уже описан — повторён здесь по невнимательности.
...
def extract_quotes(text):
    ...
    for m in re.finditer(r'«([^«»]{%d,})»' % MIN_LEN, text, re.S):
        ...
        # Снимать его обязательно: в этих справочниках выделять решающие слова
        # прямо в цитате — основной приём, и без снятия такая цитата не
        # находилась в корпусе вовсе. Она молча уходила в счётчик «вне
        # корпуса», её адрес не проверялся никогда, а итог всё равно печатал
        # «расхождений 0». По справочнику о субсидиарке так терялись 19 цитат
        # из 142 — снято 2026-08-04 при приёмке захода 6.
        q = q.replace('**', '').replace('__', '')
        q = re.sub(r'^\s*(?:\.{2,}|…)\s*|\s*(?:\.{2,}|…)\s*$', ' ', q).strip()
        if len(q) < MIN_LEN:
            continue
        line = text.count('\n', 0, m.start()) + 1
        ...
        # Граница абзаца обрывает подводку — но не всегда. Законный приём этих
        # справочников: подводка абзацем, а сама цитата следующим блоком
        # («Абзац второй п. 34(1) — шесть вопросов:» ↵↵ «> „…“»). Там адрес
        # действительно лежит за границей, и рубить нельзя.
        # Отличает их длина остатка ПОСЛЕ границы: у подводки-блоком он почти
        # пуст, у чужого абзаца — своя проза со своим смыслом. Без этого
        # «абзац третий п. 2 той же статьи» из абзаца выше приписывался цитате
        # ниже (ложное расхождение по справочнику об убытках, стр. 922);
        # со слепой отсечкой терялось 30 сверенных адресов по оспариванию.
        para = list(re.finditer(r'\n\s*\n', before))
        if para and len(before) - para[-1].end() >= 40:
            before = before[para[-1].end():]
...
def check(path, acts, with_punkt=False):
    ...
    ambiguous_ex = []
    outside_ex = []               # для --outside: что процитировано мимо корпуса
    print('=' * 72)
    ...
    # Цитаты мимо корпуса — не ошибка, но и не проверка: это ровно тот
    # материал, о котором машина не может сказать ничего, и потому он обязан
    # быть назван в разделе «открытые позиции» справочника. Флагом, а не
    # молча в счётчике, — иначе «расхождений 0» читается шире, чем значит.
    if '--outside' in sys.argv and outside_ex:
        print('\n' + '- ' * 30)
        print('ЦИТАТЫ ВНЕ КОРПУСА (%d) — машиной не проверяются вовсе:' % outside)
        for line, quote in outside_ex:
            print('  стр. %-5d «%s…»' % (line, quote[:95].replace('\n', ' ')))
