...
- Notion-конфиг настроен: файл `~/.vassal/notion-config.yaml` существует, содержит ID баз `cases` и `judges` (см. шаблон в [scripts/notion-init.md](../../scripts/notion-init.md)).
...
1. Прочитай `~/.vassal/notion-config.yaml`. Если файла нет -- предупреди Сюзерена шаблоном:
   ```
   ...
   Что нужно от Сюзерена: запустить bootstrap по scripts/notion-init.md
       (создать две базы Cases + Judges в Notion и записать их ID в ~/.vassal/notion-config.yaml).
...
3. **Feature detection Notion MCP** (по паттерну `shared/conventions.md` → «Внешние зависимости и fallback»):
...
- Notion-конфиг прочитан и валиден ИЛИ скилл вышел с явным предупреждением Сюзерену (без изменений).
...
Если в `~/.vassal/notion-config.yaml` нет конфига -- НЕ предлагать (см. предусловия notion-sync).
```
